...
public class Depurant {
    ...
    public static void main(String[] args) {
        ...
        int max = -999; //El signe de < sa de canviar per >
        for (int i = 0; i < taula.length; i++) {
            if (taula[i]>max)
                max = taula[i];
        }
        ...
        int cont = 0;// En el if no es unr || es un && perque san de complir les dues condicions
        for (int i = 0; i < taula.length; i++) {
        ...
        int parell = 0;//El if estava malament perque no recorria l'array
        for (int i = 0; i < taula.length; i++) {
            if (taula[i]%2==0)
                ++parell;
        }
